...
def test_mozart_analyzer():
    ...
    test_audio_path = "temp_audio/test_track_1.mp3"
    
    if os.path.exists(test_audio_path):
        print(f"Testing with existing audio file: {test_audio_path}")
        
        # Test just loading the audio first
        try:
            print("Testing audio loading...")
            audio, sr = mozart_analyzer.load_audio(test_audio_path)
            if audio is not None:
                ...
                try:
                    ...
                    print(f"✅ Tempo extraction successful: {tempo} BPM")
                    
                    # Test spectral features
                    spectral_centroid = librosa.feature.spectral_centroid(y=audio, sr=sr)[0]
                    print(f"✅ Spectral centroid extraction successful: {np.mean(spectral_centroid):.2f}")
                ...
                print("Testing full Mozart analysis...")
...
def test_analyzer_components():
    ...
    # Initialize configuration
    config = Config()  # Ensure config is an instance
    print(f"Config instance: {config}")
    print(f"Temp directory from config: {config.get('audio', 'temp_directory')}")
